$(() => {
  ...
  function addPolls(jsonData, showResults = false, successMessage = false) {
    clearPolls();
    ...
    `);
      const createdPoll = poll.createPoll(jsonData[0], showResults, successMessage);
      addPoll(createdPoll);
    } else {
      if (showResults) {
        $polls.append( `
      ...
      for (const pollData of jsonData) {
        const createdPoll = poll.createPoll(pollData, showResults, successMessage);
